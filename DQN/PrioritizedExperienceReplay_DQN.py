...
from PrioritizedReplayMemory import ReplayMemory
...
class PER_DQN:
    ...
    def get_loss_and_update_priority(self):
        state_batch, action_batch, reward_batch, next_state_batch, \
            importance_batch, sample_index = self.memory_buffer.get_batch()
        ...
        expected_state_action_values = (reward_batch + self.discount_factor * next_state_value).unsqueeze(1)

        # update priority
        # from tensor[batch_size, 1] to 1D list
        error = torch.abs(state_action_value - expected_state_action_values)
        error = error.flatten().detach().tolist()
        self.memory_buffer.set_priorities(sample_index, error)

        # MSE Loss function
        loss = (F.mse_loss(state_action_value, expected_state_action_values) * importance_batch).mean()
        return loss
        ...
        for episode in range(self.num_episode):
            ...
            for duration in count():
                ...
                For each steps:
                    ...
                    Get experience sample from reply buffer.
                        Compute importance-sampling weights.
                        Sample batch with importance-sampling weights with from replay memory.
                    ...
                    Calculate loss between output Q-values and target Q-values.
                        ...
                        Compute importance : wi = (1/N + 1/sample-priority) ** scale
                ...
                if self.memory_buffer.enough_samples():
                    loss = self.get_loss_and_update_priority()
...
def main():
    ...
    estimator = PER_DQN(env, 1000, -100)
    estimator.train()
